...
func imExecution(pI Instruction) {
	...
	case pI.opcode >= 1940 && pI.opcode <= 1943:
		...
		if pI.shiftCode == 0 {
			temp2 := uint64(pI.aluResult) & 0xFFFFFFFFFFFF0000
			pI.aluResult = int(temp2 | temp)
		}
		if pI.shiftCode == 1 {
			temp2 := uint64(pI.aluResult) & 0xFFFFFFFF0000FFFF
			pI.aluResult = int(temp2 | temp)
		}
		if pI.shiftCode == 2 {
			temp2 := uint64(pI.aluResult) & 0xFFFF0000FFFFFFFF
			pI.aluResult = int(temp2 | temp)
		}
		if pI.shiftCode == 3 {
			temp2 := uint64(pI.aluResult) & 0x0000FFFFFFFFFFFF
			pI.aluResult = int(temp2 | temp)
		}
	default:
